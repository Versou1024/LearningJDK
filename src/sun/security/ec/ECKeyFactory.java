...
public final class ECKeyFactory extends KeyFactorySpi
{
    ...
    throws InvalidKeyException
  {
    ...
    if (paramECKey instanceof ECPrivateKey)
    {
    ...
    label40: String str = ((Key)paramECKey).getAlgorithm();
    if (!(str.equals("EC")))
    ...
    throws InvalidKeyException
  {
    ...
    if ("X.509".equals(paramPublicKey.getFormat()))
    {
      ...
      return new ECPublicKeyImpl(localObject);
    }
    ...
    throws InvalidKeySpecException
  {
    ...
    if (paramKey instanceof ECPublicKey)
    {
      ...
      if (ECPublicKeySpec.class.isAssignableFrom(paramClass))
        return new ECPublicKeySpec(((ECPublicKey)localObject).getW(), ((ECPublicKey)localObject).getParams());
    ...
    if (paramKey instanceof ECPrivateKey)
    {
      ...
      if (ECPrivateKeySpec.class.isAssignableFrom(paramClass))
      {
        ...
        return new ECPrivateKeySpec(((ECPrivateKey)localObject).getS(), ((ECPrivateKey)localObject).getParams());
      }
  ...
  static
  {
    1 local1 = new Provider("SunEC-Internal", 1D, null)
    {
    };
    AccessController.doPrivileged(new PrivilegedAction(local1)
    {
      public Void run()
      {
    ...
    try
    {
      INSTANCE = KeyFactory.getInstance("EC", local1);
    }
    catch (NoSuchAlgorithmException localNoSuchAlgorithmException)
    {
      throw new RuntimeException(localNoSuchAlgorithmException);
    }
    ecInternalProvider = local1;
  }
}
...
class NativeObject
{
  ...
  {
    return new NativeObject(paramInt + this.address);
  }
  ...
  void putObject(int paramInt, NativeObject paramNativeObject)
  {
    ...
    case 8:
      ...
      break;
    case 4:
      putInt(paramInt, (int)(paramNativeObject.address & 0xFFFFFFFF));
      break;
    default:
      throw new InternalError("Address size not supported");
    }
  ...
  {
    return unsafe.getByte(paramInt + this.address);
  }

  final void putByte(int paramInt, byte paramByte)
  {
    unsafe.putByte(paramInt + this.address, paramByte);
  }

  final short getShort(int paramInt)
  {
    return unsafe.getShort(paramInt + this.address);
  }

  final void putShort(int paramInt, short paramShort)
  {
    unsafe.putShort(paramInt + this.address, paramShort);
  }

  final char getChar(int paramInt)
  {
    return unsafe.getChar(paramInt + this.address);
  }

  final void putChar(int paramInt, char paramChar)
  {
    unsafe.putChar(paramInt + this.address, paramChar);
  }

  final int getInt(int paramInt)
  {
    return unsafe.getInt(paramInt + this.address);
  }

  final void putInt(int paramInt1, int paramInt2)
  {
    unsafe.putInt(paramInt1 + this.address, paramInt2);
  }

  final long getLong(int paramInt)
  {
    return unsafe.getLong(paramInt + this.address);
  }

  final void putLong(int paramInt, long paramLong)
  {
    unsafe.putLong(paramInt + this.address, paramLong);
  }

  final float getFloat(int paramInt)
  {
    return unsafe.getFloat(paramInt + this.address);
  }

  final void putFloat(int paramInt, float paramFloat)
  {
    unsafe.putFloat(paramInt + this.address, paramFloat);
  }

  final double getDouble(int paramInt)
  {
    return unsafe.getDouble(paramInt + this.address);
  }

  final void putDouble(int paramInt, double paramDouble)
  {
    unsafe.putDouble(paramInt + this.address, paramDouble);
  }

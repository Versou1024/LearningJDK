...
final class Order1 extends Curve
{
  ...
  {
    if ((this.x0 == this.x1) || (paramDouble <= this.y0))
      return this.x0;
    if (paramDouble >= this.y1)
      return this.x1;
    return (this.x0 + (paramDouble - this.y0) * (this.x1 - this.x0) / (this.y1 - this.y0));
  }
  ...
  public double TforY(double paramDouble)
  {
    ...
    if (paramDouble >= this.y1)
      return 1D;
    return ((paramDouble - this.y0) / (this.y1 - this.y0));
  }
  ...
  public int compareTo(Curve paramCurve, double[] paramArrayOfDouble)
  {
    ...
    paramArrayOfDouble[1] = Math.min(Math.min(paramArrayOfDouble[1], this.y1), localOrder1.y1);
    if (paramArrayOfDouble[1] <= paramArrayOfDouble[0])
      throw new InternalError("backstepping from " + paramArrayOfDouble[0] + " to " + paramArrayOfDouble[1]);
    if (this.xmax <= localOrder1.xmin)
      return ((this.xmin == localOrder1.xmax) ? 0 : -1);
    if (this.xmin >= localOrder1.xmax)
      return 1;
    double d1 = this.x1 - this.x0;
    double d2 = this.y1 - this.y0;
    double d3 = localOrder1.x1 - localOrder1.x0;
    double d4 = localOrder1.y1 - localOrder1.y0;
    double d5 = d3 * d2 - d1 * d4;
    if (d5 != 0D)
    {
      double d7 = (this.x0 - localOrder1.x0) * d2 * d4 - this.y0 * d1 * d4 + localOrder1.y0 * d3 * d2;
      d6 = d7 / d5;
      if (d6 <= paramArrayOfDouble[0])
      {
        d6 = Math.min(this.y1, localOrder1.y1);
      }
      else
      {
        if (d6 < paramArrayOfDouble[1])
          paramArrayOfDouble[1] = d6;
        d6 = Math.max(this.y0, localOrder1.y0);
      }
    }
    else
    {

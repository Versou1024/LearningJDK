  ...
  protected ImageConsumerQueue nextConsumer(ImageConsumerQueue paramImageConsumerQueue)
  {
    synchronized (this.source)
    {
      if (!(this.aborted))
        break label18;
      return null;
      label18: paramImageConsumerQueue = (paramImageConsumerQueue == null) ? this.queue : paramImageConsumerQueue.next;
      while (true)
      {
        if (paramImageConsumerQueue == null)
          break label57;
        if (paramImageConsumerQueue.interested)
          return paramImageConsumerQueue;
        label57: paramImageConsumerQueue = paramImageConsumerQueue.next;
      }
    }
    return null;
  }

    ...
    implements SeekableByteChannel, GatheringByteChannel, ScatteringByteChannel
{
    ...
    {
        // 打开或创建文件，返回文件通道以访问文件。
        //  options参数确定文件的打开方式。 READ和WRITE选项确定是否应打开文件进行读取和/或写入。如果数组中不包含任何选项（或APPEND选项），则打开文件以供读取。默认情况下，读取或写入从文件的开头开始。
        //  除了READ和WRITE之外，还可能存在以下选项：
        // 选项  描述
        // APPEND 如果存在此选项，则打开文件以进行写入，并且每次调用通道的write方法首先将位置推进到文件的末尾，然后写入请求的数据。位置的提升和数据的写入是否在单个原子操作中完成是系统相关的，因此未指定。此选项不能与READ或TRUNCATE_EXISTING选项一起使用。
        // TRUNCATE_EXISTING 如果存在此选项，则现有文件将被截断为 0 字节大小。当文件只为阅读而打开时，此选项将被忽略。
        // CREATE_NEW 如果存在此选项，则创建一个新文件，如果该文件已存在则失败。创建文件时，检查文件是否存在以及如果文件不存在则创建文件对于其他文件系统操作而言是原子操作。当文件只为阅读而打开时，此选项将被忽略。
        // CREATE 如果存在此选项，则打开现有文件（如果存在），否则创建新文件。创建文件时，检查文件是否存在以及如果文件不存在则创建文件对于其他文件系统操作而言是原子操作。如果CREATE_NEW选项也存在或文件打开仅用于读取，则忽略此选项。
        // DELETE_ON_CLOSE 当此选项存在时，实现会尽最大努力尝试在由close方法关闭时删除文件。如果未调用close方法，则在 Java 虚拟机终止时尽最大努力删除文件。
        // SPARSE 创建新文件时，此选项暗示新文件将是稀疏的。不创建新文件时忽略此选项。
        // SYNC 要求对文件内容或元数据的每次更新都同步写入底层存储设备。 （请参阅同步 I/O 文件完整性）。
        // DSYNC 要求对文件内容的每次更新都同步写入底层存储设备。 （请参阅同步 I/O 文件完整性）。
